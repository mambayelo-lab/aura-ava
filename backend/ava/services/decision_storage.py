"""Service de stockage pour les interviews décisionnelles"""
import json
...
STORAGE_DIR = Path(__file__).parent.parent / "data" / "decision_interviews"
STORAGE_DIR.mkdir(parents=True, exist_ok=True)

def save_decision_interview(interview: DecisionInterview) -> None:
    """Sauvegarde une interview"""
...
def read_decision_interview(interview_id: str) -> Optional[DecisionInterview]:
    """Charge une interview"""
    ...
    return DecisionInterview(**data)

def list_decision_interviews() -> List[DecisionInterview]:
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                ...
                if 'submitted_at' in data and data['submitted_at'] and isinstance(data['submitted_at'], str):
                    from datetime import datetime
                    data['submitted_at'] = datetime.fromisoformat(data['submitted_at'].replace('Z', '+00:00'))
                interviews.append(DecisionInterview(**data))
